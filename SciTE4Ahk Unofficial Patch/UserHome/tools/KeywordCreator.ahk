;
; UnofficialPatch - Keyword Creator
; Developed on 2022.04.22
;
...
Parse_Side(Index, txt) {
	if (Index = 2 || Index = 5)
		pos := RegExMatch(txt, "(style\.ahk1\.4|s4ahk\.style\.operator)=(.*)", Match)
	else if (Index = 3)
		pos := RegExMatch(txt, "(style\.ahk1\.5|s4ahk\.style\.operator)=(.*)", Match)
	else if (Index = 6)
	{
		pos := RegExMatch(txt, "(style\.ahk1\.9|s4ahk\.style\.operator)=(.*)", Match)
